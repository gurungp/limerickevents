...
public class RdsDataThisMonth extends AsyncTask<Void, Void, ResultSet>{

	private Rdsinter<ResultSet> listener;
	private static Calendar cal = Calendar.getInstance();
	private int dayOfMonth=0,endOfMonth;
	...
	public RdsDataThisMonth(Rdsinter<ResultSet> r, int id, int p){
		this.listener = r;
		eventid=id;
		prefer=p;
		...
		endOfMonth = endOfMonth-dayOfMonth;
		dateToday = fmt.format(currentDate);
		cal.add(Calendar.DATE, endOfMonth);
